...
class KITTINode(object):
    ...
    def __init__(self, dataset_path=' ',
        ...
        # loop publisher
        self._velodyne_points_pub_2 = rospy.Publisher("loop_points", PointCloud2, queue_size=1)  # 1
        pub_rotate_velody_points_topic = "/loop_nodes"
        self._nodes_pub_2 = rospy.Publisher(pub_rotate_velody_points_topic, PointCloud2, queue_size=1)  # 1
        ...
        # publish pointcloud
        # while(1):
        #     for f in bin_files[2430:2500]:   # 400 450
        for f in bin_files[2250:]:
            if rospy.is_shutdown():
                break

            self.publish_pointcloud(self._path + "/" + f, header)   # 2453
            cnt += 1
    ...
    def publish_pointcloud(self, bin_file, header):
        ...
        if int(file_name) >= 2440 and int(file_name) <= 2473:
            # if file_name == "002453":
            l_name = "000" + str(int(int(file_name) - 2055)) + ".npy"
            loop_file = self._path + "/" + l_name
            # loop_file = self._path + "/000401.npy"
            loop_file_name = loop_file.strip('.npy').split('/')[-1]
            graph_json = os.path.join(graph_base_dir, loop_file_name + ".json")
            print(graph_json)
            graph_data = json.load(open(graph_json))
            nodes_center = graph_data["centers"]
            nodes_sem = graph_data["nodes"]
            lidar_loop = np.load(loop_file)
            
            # lidar = np.row_stack((lidar_loop, lidar))
            x_loop = lidar_loop[:, 0].reshape(-1)
            y_loop = lidar_loop[:, 1].reshape(-1)
            z_loop = lidar_loop[:, 2].reshape(-1)
            z_loop = z_loop + 20
            intensity_loop = lidar_loop[:, 3].reshape(-1)  # * 255  # 0~1 -> 0~255
            # ring = lidar[:, 4].reshape(lidar.shape[0],1).astype(np.int16)
            # label = lidar_label.reshape(lidar.shape[0],1).astype(np.int16)
            sem_loop = lidar_loop[:, 4].reshape(-1).astype(np.int16)
            ins_loop = lidar_loop[:, 5].reshape(-1).astype(np.int16)
            
            # cloud = []
            cloud_loop = np.stack((x_loop, y_loop, z_loop, intensity_loop, sem_loop, ins_loop))
            # cloud = np.stack((x, y, z, intensity, sem))
            cloud = np.hstack((cloud,cloud_loop))
           
            nodes_center = np.array(nodes_center).reshape(-1, 3)  # nx3
            nodes_sem = np.array(nodes_sem).reshape(-1)
            node_x_loop = nodes_center[:, 0].reshape(-1)
            node_y_loop = nodes_center[:, 1].reshape(-1)
            node_z_loop = nodes_center[:, 2].reshape(-1)
            node_z_loop = node_z_loop + 20
            nodes_loop = np.stack((node_x_loop, node_y_loop, node_z_loop, nodes_sem))
            nodes = np.hstack((nodes, nodes_loop))
