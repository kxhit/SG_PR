...
# sequences = ["00"]
root_dir = "compare/all/"+ threshold + "/" + drop + "/"
...
for sq in sequences:
    ...
    print(output_dir)
    # load M2DP
    M2DP_base_dir = "compare/M2DP/eval/"+threshold+"/" + drop + "/"
    M2DP_pred_db = M2DP_base_dir + sq + "/" + sq + "_M2DP_db.npy"
    M2DP_gt_db = M2DP_base_dir + sq + "/" + sq + "_gt_db.npy"
    M2DP_pred = np.load(M2DP_pred_db)
    M2DP_gt = np.load(M2DP_gt_db)

    # load SC
    SC_base_dir = "compare/SC/eval/"+threshold+"/" + drop + "/"
    SC_pred_db = SC_base_dir + sq + "/" + sq + "_SC_db.npy"
    SC_gt_db = SC_base_dir + sq + "/" + sq + "_gt_db.npy"
    ...
    PV_ref_base_dir = "compare/PV_PRE/eval/"+threshold+"/" + drop + "/"
    PV_ref_pred_db = PV_ref_base_dir + sq + "/" + sq + "_PV_db.npy"
    PV_ref_gt_db = PV_ref_base_dir + sq + "/" + sq + "_gt_db.npy"
    PV_ref_pred = np.load(PV_ref_pred_db)
    PV_ref_gt = np.load(PV_ref_gt_db)

    # load PointNetVLAD trained on KITTI 1-fold
    PV_base_dir = "compare/PV_KITTI/eval/"+threshold+"/" + drop + "/"
    PV_pred_db = PV_base_dir + sq + "/" + sq + "_PV_db.npy"
    PV_gt_db = PV_base_dir + sq + "/" + sq + "_gt_db.npy"
    ...
    # load RN
    rangenet_base_dir = threshold + "/Ours_RN/" + drop + "/"
    rangenet_pred_db = rangenet_base_dir + sq + "/" + sq + "_DL_db.npy"
    rangenet_gt_db = rangenet_base_dir + sq + "/" + sq + "_gt_db.npy"
    ...
    pr_roc_metrics(output_dir, sq, pred_list, simgnn_gt, method_list)